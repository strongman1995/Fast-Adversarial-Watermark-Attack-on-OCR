...
class Model(object):
  ...
  def get_probs(self, x, **kwargs):
    ...
    if self.O_PROBS in d:
      ...
      asserts = [utils_tf.assert_greater_equal(min_prob, tf.cast(0., min_prob.dtype)),
                 utils_tf.assert_less_equal(max_prob, tf.cast(1., min_prob.dtype))]
  ...
  def get_params(self):
    ...
    scope_vars = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, self.scope + "/")
