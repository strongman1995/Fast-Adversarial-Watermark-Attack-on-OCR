...
class HWDB(Dataset):
  ...
  def __init__(self, data_dir, train_start=0, train_end=60000,
               test_start=0, test_end=10000,
               center=False, max_val=1.):
    kwargs = locals()
    if '__class__' in kwargs:
      del kwargs['__class__']
    super(HWDB, self).__init__(kwargs)
    x_train, y_train, x_test, y_test = data_hwdb(data_dir, train_start=train_start, train_end=train_end,
...
def data_hwdb(data_dir, train_start=0, train_end=50000,
              test_start=0, test_end=10000):
  assert isinstance(train_start, int)
  assert isinstance(train_end, int)
  assert isinstance(test_start, int)
  ...
  def get_image_names_labels(data_dir):
    truncate_path = os.path.join(data_dir, '%05d' % FLAGS.charset_size)
    print(truncate_path)
    image_names = []
    ...
    labels = [int(file_name[len(data_dir):].split(os.sep)[1]) for file_name in image_names]
    return image_names, labels

  train_image_names, train_labels = get_image_names_labels(os.path.join(data_dir, 'train'))
  test_image_names, test_labels = get_image_names_labels(os.path.join(data_dir, 'test'))

  train_image_names = train_image_names[train_start:train_end]
  y_train = train_labels[train_start:train_end]
  test_image_names = test_image_names[test_start:test_end]
  y_test = test_labels[test_start:test_end]

  def get_images(file_list):
    images = []
    for file_name in file_list:
      temp_image = Image.open(file_name).convert('L')
      temp_image = temp_image.resize((FLAGS.image_size, FLAGS.image_size), Image.ANTIALIAS)
      images += [np.asarray(temp_image) / 255.0]
    return np.array(images).reshape([-1, 64, 64, 1])

  x_train = get_images(train_image_names)
  y_train = np_utils.to_categorical(y_train, FLAGS.charset_size)
  x_test = get_images(test_image_names)
  y_test = np_utils.to_categorical(y_test, FLAGS.charset_size)
  return x_train, y_train, x_test, y_test