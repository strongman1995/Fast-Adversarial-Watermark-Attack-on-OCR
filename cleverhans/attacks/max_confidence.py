...
class MaxConfidence(Attack):
  ...
  An attack designed for use against models that use confidence thresholding as a defense.
  If the underlying optimizer is optimal, this attack procedure gives the optimal failure rate for every confidence threshold t > 0.5.
  ...
  def attack(self, x, true_y):
    ...
    :param x: The input
    :param true_y: The correct label for `x`.
                   This attack aims to produce misclassification.
    """
    adv_x_cls = []
    prob_cls = []
    m = tf.shape(x)[0]
