...
class CWL2(object):
    def __init__(self, sess, model, batch_size, confidence, targeted,
        ...
        self.tlab = tf.Variable(np.zeros(tuple([batch_size] + label_shape)), dtype=tf_dtype, name='tlab')
        ...
        self.assign_tlab = tf.placeholder(tf_dtype, tuple([batch_size] + label_shape), name='assign_tlab')
        ...
        real = reduce_sum(self.tlab * self.output, axis=-1)
        other = reduce_max((1 - self.tlab) * self.output - self.tlab * 10000, axis=-1)
            ...
            for iteration in range(self.MAX_ITERATIONS):
                ...
                for e, (l2, sc, ii) in enumerate(zip(l2s, scores, nimg)):
                    ...
                    if l2 < bestl2[e] and compare(sc, lab):
                        bestl2[e] = l2
                        bestscore[e] = np.argmax(sc, axis=-1)

                    if l2 < o_bestl2[e] and compare(sc, lab):
                        o_bestl2[e] = l2
                        o_bestscore[e] = np.argmax(sc, axis=-1)
                        o_bestattack[e] = ii
