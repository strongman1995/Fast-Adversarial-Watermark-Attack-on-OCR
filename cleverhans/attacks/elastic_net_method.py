...
class ElasticNetMethod(Attack):
    ...
    def generate(self, x, **kwargs):
        ...
        attack = EAD(self.sess, self.model, self.beta, self.decision_rule, self.batch_size, self.confidence,
                     'y_target' in kwargs, self.learning_rate, self.binary_search_steps, self.max_iterations,
                     self.abort_early, self.initial_const, self.clip_min, self.clip_max, nb_classes,
                     x.get_shape().as_list()[1:], labels.get_shape().as_list()[1:])
    ...
    def parse_params(self, y=None, y_target=None, beta=1e-2, decision_rule='EN', batch_size=1, confidence=0,
                     learning_rate=1e-2,
                     binary_search_steps=9, max_iterations=1000, abort_early=False, initial_const=1e-3, clip_min=0,
                     clip_max=1):
        """
...
class EAD(object):
    ...
    def attack(self, imgs, targets):
        ...
        for i in range(0, len(imgs) // batch_size):
            _logger.debug(("Running EAD attack on instance %s of %s", i * batch_size, len(imgs)))
        ...
        if len(imgs) % batch_size != 0:
            ...
            _logger.debug(("Running EAD attack on instance %s of %s", last_elements, len(imgs)))
        ...
        for outer_step in range(self.BINARY_SEARCH_STEPS):
            ...
            for iteration in range(self.MAX_ITERATIONS):
                ...
                l, l2s, l1s, crit, scores, nimg = self.sess.run(
                    [self.loss, self.l2dist, self.l1dist, self.crit, self.output, self.newimg])
                if iteration % ((self.MAX_ITERATIONS // 10) or 1) == 0:
                    _logger.debug("    Iteration {} of {}: loss={:.3g} l2={:.3g} l1={:.3g} f={:.3g}"
                                  .format(iteration, self.MAX_ITERATIONS, l, np.mean(l2s), np.mean(l1s), np.mean(scores)))
            ...
            _logger.debug(self.crit_p + " Mean successful distortion: {:.4g}".format(mean))
