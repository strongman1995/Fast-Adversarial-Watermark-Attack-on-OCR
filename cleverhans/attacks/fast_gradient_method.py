...
class FastGradientMethod(Attack):
    ...
    def generate(self, x, **kwargs):
        ...
        return fgm(x, self.model.get_logits(x), y=labels, eps=self.eps, ord=self.ord, clip_min=self.clip_min, clip_max=self.clip_max,
                   targeted=(self.y_target is not None), sanity_checks=self.sanity_checks)

    def parse_params(self, eps=0.3, ord=np.inf, y=None, y_target=None, clip_min=None, clip_max=None, sanity_checks=True, **kwargs):
        """
...
def fgm(x, logits, y=None, eps=0.3, ord=np.inf, clip_min=None, clip_max=None, targeted=False, sanity_checks=True):
    ...
    if y is None:
        ...
        preds_max = tf.reduce_max(logits, axis=-1, keepdims=True)
    ...
    if loss_type == "softmax":
        ...
        sparse_labels = utils_tf.dense_to_sparse(y)
        loss = tf.nn.ctc_loss(labels=sparse_labels, inputs=logits, sequence_length=y.get_shape().as_list()[-1:], time_major=False)
...
def optimize_linear(grad, eps, ord=np.inf):
    ...
    if ord == np.inf:
        ...
        # It applies only because `optimal_perturbation` is the output of a `sign` op, which has zero derivative anyway.
        ...
        square = tf.maximum(avoid_zero_div, tf.reduce_sum(tf.square(grad), axis=red_ind, keepdims=True))
...
def optimize_linear_pos(grad, eps, ord=np.inf, pert_type='all'):
    """
    Solves for the optimal input to a linear function under a norm constraint.
    ...
    if ord == np.inf:
        ...
        if pert_type == 'all':
            ...
            optimal_perturbation = tf.minimum(optimal_perturbation, 0) # pos noise
        elif pert_type == 'neg':
            optimal_perturbation = tf.maximum(optimal_perturbation, 0) # neg noise
        else:
            raise Exception("pert_type must be 'all', 'pos' or 'neg'")
        # The following line should not change the numerical results.
        # It applies only because `optimal_perturbation` is the output of a `sign` op, which has zero derivative anyway.
        # It should not be applied for the other norms, where the perturbation has a non-zero derivative.
        optimal_perturbation = tf.stop_gradient(optimal_perturbation)
