...
class SaliencyMapMethod(Attack):
  ...
  def generate(self, x, **kwargs):
    ...
    if self.symbolic_impl:
      ...
      x_adv = jsma_symbolic(x, model=self.model, y_target=self.y_target, theta=self.theta, gamma=self.gamma,
  ...
  def parse_params(self, theta=1., gamma=1., clip_min=0., clip_max=1., y_target=None, symbolic_impl=True, **kwargs):
    """
...
def jsma_symbolic(x, y_target, model, theta, gamma, clip_min, clip_max):
  ...
  # nb_features = int(np.product([32, 18, 1]))
  ...
  # Loop variables
  # x_in: the tensor that holds the latest adversarial outputs that are in progress.
  # y_in: the tensor for target labels
  # domain_in: the tensor that holds the latest search domain
  # cond_in: the boolean tensor to show if more iteration is needed for generating adversarial samples
  ...
  # Run loop to do JSMA
  x_adv, _, _, _, _ = tf.while_loop(condition, body, [x, y_target, search_domain, 0, True], parallel_iterations=1)
