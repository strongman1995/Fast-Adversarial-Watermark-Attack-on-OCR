...
class Attack(object):
  ...
  def __init__(self, model, sess=None, dtypestr='float64', **kwargs):
  ...
  def construct_graph(self, fixed, feedable, x_val, hash_key):
    ...
    for name, value in feedable.items():
      ...
      if isinstance(value, np.ndarray):
        ...
        raise ValueError("Could not identify type of argument " + name + ": " + str(value))

    # x is a special placeholder we always want to have
    x_shape = [None] + list(x_val.shape)[1:]
    x = tf.placeholder(self.tf_dtype, shape=x_shape, name="x_np")
