...
class LBFGS(Attack):
    ...
    def __init__(self, model, sess, dtypestr='float32', **kwargs):
        ...
        self.structural_kwargs = ['batch_size', 'binary_search_steps', 'max_iterations', 'initial_const', 'clip_min', 'clip_max']
    ...
    def generate(self, x, **kwargs):
        ...
        attack = LBFGS_impl(self.sess, x, self.model.get_logits(x), self.y_target, self.binary_search_steps, self.max_iterations,
                            self.initial_const, self.clip_min, self.clip_max, nb_classes, self.batch_size)
    ...
    def parse_params(self, y_target=None, batch_size=1, binary_search_steps=5, max_iterations=1000, initial_const=1e-2, clip_min=0, clip_max=1):
        """
        :param y_target: (optional) A tensor with the one-hot target labels.
        :param batch_size: The number of inputs to include in a batch and process simultaneously.
        :param binary_search_steps: The number of times we perform binary search to find the optimal tradeoff-constant
                                    between norm of the purturbation and cross-entropy loss of classification.
...
class LBFGS_impl(object):
    ...
    :param binary_search_steps: The number of times we perform binary search to find the optimal tradeoff-constant
                                between norm of the purturbation and cross-entropy loss of classification.
    :param max_iterations: The maximum number of iterations.
    :param initial_const: The initial tradeoff-constant to use to tune the relative importance of size of the purturbation
                          and cross-entropy loss of the classification.
    ...
    def __init__(self, sess, x, logits, targeted_label, binary_search_steps, max_iterations,
                 initial_const, clip_min, clip_max, nb_classes, batch_size):
        self.sess = sess
        self.x = x
        self.logits = logits
        ...
        # self.shape = tuple([self.batch_size, 32, 18, 1])
        self.ori_img = tf.Variable(np.zeros(self.shape), dtype=tf_dtype, name='ori_img')
        self.const = tf.Variable(np.zeros(self.batch_size), dtype=tf_dtype, name='const')
        ...
        self.loss = reduce_sum(self.score * self.const) + self.l2dist  # loss = softmax_cross_entropy + l2dist
    ...
    def attack(self, x_val, targets):
        ...
        def lbfgs_objective(adv_x, self, targets, oimgs, CONST):
            ...
            loss, grad = self.sess.run([self.loss, self.grad], feed_dict={self.x: adv_x.reshape(oimgs.shape),
                                                                          self.targeted_label: targets,
                                                                          self.ori_img: oimgs,
                                                                          self.const: CONST})
            # grad = self.sess.run(self.grad, feed_dict={self.x: adv_x.reshape(oimgs.shape),
            #                                            self.targeted_label: targets,
            #                                            self.ori_img: oimgs,
            #                                            self.const: CONST})
        ...
        for outer_step in range(self.binary_search_steps):
            _logger.debug("  Binary search step %s of %s", outer_step, self.binary_search_steps)

            # The last iteration (if we run many steps) repeat the search once.
            if self.repeat and outer_step == self.binary_search_steps - 1:
                CONST = upper_bound
            # optimization function
            adv_x, _, __ = fmin_l_bfgs_b(lbfgs_objective, oimgs.flatten().astype(float),
            ...
            plt.imsave(f"adv_{outer_step}.jpg", adv_x.reshape([adv_x.shape[1], adv_x.shape[2]]))
            assert np.amax(adv_x) <= self.clip_max and \
                   np.amin(adv_x) >= self.clip_min, 'fmin_l_bfgs_b returns are invalid'

            # adjust the best result (i.e., the adversarial example with the
            # smallest perturbation in terms of L_2 norm) found so far
            preds = np.atleast_1d(utils_tf.model_argmax(self.sess, self.x, self.logits, adv_x))
            ...
            for e, (l2, pred, ii) in enumerate(zip(l2s, preds, adv_x)):
                print("pred:", pred)
                print("targets:", np.argmax(targets[e], axis=-1))
                print("l2", l2, "o_bestl2[e]", o_bestl2[e])
                print(pred == np.argmax(targets[e], axis=-1))
                if l2 < o_bestl2[e]:
                    res = pred == np.argmax(targets[e], axis=-1)
            ...
            for e in range(self.batch_size):
                ...
                if isinstance(res, np.ndarray):
                    res = np.sum(1 - res) == 0
                # if preds[e] == np.argmax(targets[e], axis=-1):
                if res:
            ...
            _logger.debug("  Successfully generated adversarial examples on %s of %s instances.", sum(upper_bound < 1e9), self.batch_size)
