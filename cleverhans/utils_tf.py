...
def clip_eta(eta, ord, eps):
  ...
  if ord == np.inf:
    ...
    if ord == 1:
      raise NotImplementedError("The expression below is not the correct way to project onto the L1 norm ball.")
      # norm = tf.maximum(avoid_zero_div, educe_sum(tf.abs(eta), reduc_ind, keep_dims=True))
    elif ord == 2:
      # avoid_zero_div must go inside sqrt to avoid a divide by zero
      # in the gradient through this operation
      norm = tf.sqrt(tf.maximum(avoid_zero_div, reduce_sum(tf.square(eta), reduc_ind, keep_dims=True)))
